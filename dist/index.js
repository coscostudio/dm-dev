...
(() => {
  ...
  var ensurePersistentShell = () => {
    ...
    let nav = wrapper.querySelector(`${NAV_SELECTOR}[${NAV_PERSIST_ATTR}]`) || wrapper.querySelector(NAV_SELECTOR) || container?.querySelector(NAV_SELECTOR) || null;
    if (nav) {
    ...
    wrapper.querySelectorAll(NAV_SELECTOR).forEach((node) => {
      if (nav && node === nav) {
        return;
      }
      node.remove();
    ...
    if (container) {
      container.querySelectorAll(NAV_SELECTOR).forEach((node) => {
        if (nav && node === nav) {
          return;
        }
        node.remove();
