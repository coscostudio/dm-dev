...
(() => {
  ...
  var getActiveContainer = () => document.querySelector(BARBA_CONTAINER_SELECTOR);
  var closeHoverSources = /* @__PURE__ */ new Set();
  var closeHoverTimeouts = /* @__PURE__ */ new WeakMap();
  var closeHoverTransitions = /* @__PURE__ */ new WeakMap();
  var getBaseDrawerGap = () => {
    const root = document.documentElement;
    if (!root.dataset.drawerGapBase) {
      const computedGap = window.getComputedStyle(root).getPropertyValue("--drawer-gap").trim();
  ...
  var getNavTransitionParts = (sourceElement) => {
    const root = document.documentElement;
    const readValue = (element) => window.getComputedStyle(element).getPropertyValue("--nav-transition").trim();
  ...
  var cacheHoverTransition = (element) => {
  ...
  var restoreHoverTransition = (element) => {
  ...
  var applyHoverTransition = (element, properties, transition) => {
    cacheHoverTransition(element);
    const computed = window.getComputedStyle(element);
    const { duration, timing, delay } = transition;
    const baseProperties = computed.transitionProperty.split(",").map((prop) => prop.trim()).filter(Boolean);
    const hasAll = baseProperties.includes("all");
    const mergedProperties = hasAll ? "all" : Array.from(/* @__PURE__ */ new Set([...baseProperties.filter((prop) => prop !== "none"), ...properties])).filter(Boolean).join(", ");
  ...
  var setCloseHoverSource = (source, isActive) => {
    if (isActive) {
  ...
  var applyCloseHoverState = (isHovering) => {
