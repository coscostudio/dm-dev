...
(() => {
  ...
  var initNavInteractions = () => {
    ...
    const handleLinkClick = (e) => {
      ...
      const isPeripheral = href && (href.includes("verkada") || href.includes("film") || href.includes("about") || href.includes("work"));
      if (isPeripheral) {
        return;
      }
      closeNav();
  ...
  var NAV_SELECTOR = ".drawer";
  var NAV_WRAPPER_SELECTOR = ".drawer-wrapper";
  var NAV_SHELL_SELECTOR = `${NAV_WRAPPER_SELECTOR}, ${NAV_SELECTOR}`;
  var NAV_PERSIST_ATTR = "data-nav-persistent";
  var CLOSE_PERSIST_ATTR = "data-close-persistent";
  var PERIPHERAL_BODY_CLASS = "is-in-peripheral";
  var TRANSITIONING_PERIPHERAL_CLASS = "is-transitioning-peripheral";
  var TRANSITION_DURATION = 700;
  var CONTENT_FADE_DURATION = 320;
  var DRAWER_SHRINK_DELAY = 0;
  var EASING = "cubic-bezier(0.4, 0, 0.2, 1)";
  var DEBUG_TRANSITIONS = false;
  var peripheralToHomeGate = null;
  var peripheralToHomeCleanup = null;
  var LOOP_SLIDER_SNAP_ATTR = "data-loop-slider-snap";
  var BARBA_CONTAINER_SELECTOR = '[data-barba="container"]';
  var PROJECT_WRAPPER_SELECTOR = ".project-wrapper";
  var HOME_CONTENT_SELECTOR = ".home-main";
  var ensurePersistentShell = () => {
    const wrapper = document.querySelector(BARBA_WRAPPER_SELECTOR);
    if (!wrapper) {
      return;
    }
    const container = document.querySelector(BARBA_CONTAINER_SELECTOR);
    let navShell = wrapper.querySelector(`${NAV_WRAPPER_SELECTOR}[${NAV_PERSIST_ATTR}]`) || wrapper.querySelector(NAV_WRAPPER_SELECTOR) || wrapper.querySelector(`${NAV_SELECTOR}[${NAV_PERSIST_ATTR}]`) || wrapper.querySelector(NAV_SELECTOR) || container?.querySelector(`${NAV_WRAPPER_SELECTOR}[${NAV_PERSIST_ATTR}]`) || container?.querySelector(NAV_WRAPPER_SELECTOR) || container?.querySelector(`${NAV_SELECTOR}[${NAV_PERSIST_ATTR}]`) || container?.querySelector(NAV_SELECTOR) || null;
    if (navShell) {
    ...
    const pruneShellNodes = (root) => {
      if (!root || !navShell) {
        return;
      }
      root.querySelectorAll(NAV_SHELL_SELECTOR).forEach((node) => {
    ...
    let close = wrapper.querySelector(`${CLOSE_SELECTOR}[${CLOSE_PERSIST_ATTR}]`) || wrapper.querySelector(CLOSE_SELECTOR) || container?.querySelector(CLOSE_SELECTOR) || null;
    if (close) {
    ...
    wrapper.querySelectorAll(CLOSE_SELECTOR).forEach((node) => {
      if (close && node === close) {
        return;
      }
      node.remove();
    ...
    if (container) {
      container.querySelectorAll(CLOSE_SELECTOR).forEach((node) => {
        if (close && node === close) {
          return;
        }
        node.remove();
  ...
  var fadeOutElement = async (element, duration = CONTENT_FADE_DURATION) => {
  ...
  var getFadeTarget = (container) => {
    if (!container) {
      return null;
    }
    return container.querySelector(PROJECT_WRAPPER_SELECTOR) ?? container.querySelector(HOME_CONTENT_SELECTOR) ?? container;
  };
  var debugLog = (label, payload) => {
  ...
  var startTransitionDebugObserver = () => {
    if (!DEBUG_TRANSITIONS) {
      return () => {
      };
    }
    const targets = [
      document.body,
      document.querySelector(NAV_SELECTOR),
      document.querySelector(BARBA_CONTAINER_SELECTOR),
    ...
    const observer = new MutationObserver((mutations) => {
      const summaries = mutations.map((mutation) => ({
        type: mutation.type,
        target: mutation.target instanceof HTMLElement ? mutation.target.className || mutation.target.tagName : String(mutation.target),
      ...
      transitions: [
        {
          ...
          leave: async ({ current, next, trigger }) => {
            ...
            const nextHref = next?.url?.href ?? (trigger instanceof Element ? trigger.getAttribute("href") : null);
            onBeforeLeave?.({
              currentNamespace,
              nextNamespace,
  ...
  var LOOP_SLIDER_CONFIG = {
    ...
    minOpacity: 0.2,
    safeZoneBuffer: 48,
    // 3rem buffer outside the viewport
    initialOffset: 0.2
    // 20% of viewport height starting "scrolled up"
  };
  ...
  var LoopSliderInstance = class {
    ...
    measure() {
      ...
      this.slides.forEach((slide) => {
        ...
        const progressTop = clamp2((itemCenter + (itemHeight / 2 + buffer)) / falloffDistance, 0, 1);
        const progressBottom = clamp2(
